...
* **Frontend:** Aplicación web moderna en React (construida con Vite).
...
3. Ve a la pestaña "Solidity Compiler". En **Advanced Configurations**, asegúrate de que el **EVM version** esté configurado en `london` (Ganache no soporta versiones más modernas del EVM).
...
2. **Edita el archivo `.env`** para que contenga tu propia configuración. Debes llenar:
   ...
   * `ADMIN_PRIVATE_KEY` y `ADMIN_ADDRESS`: Ve a Ganache, haz clic en el icono de llave de la primera cuenta y copia la clave privada y su dirección correspondiente.
